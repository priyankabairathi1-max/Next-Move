...
export async function onRequestPost(context) {
  ...
  if (missing.length) {
    ...
    // Don't fail the whole submission just because the sheet write failed —
    // the person still gets their result either way. Just log it.
  ...
  try {
    ...
    // Same reasoning — don't block the person's result over a storage failure.
    return jsonResponse({ ok: false, warning: 'Could not save lead' }, 200);
  }
}

// ---- Google Sheets write, via a hand-signed service-account JWT ----

async function appendRowToSheet(env, rowValues) {
  const accessToken = await getGoogleAccessToken(env);
  const range = 'A1'; // append() finds the next empty row on the first sheet automatically
